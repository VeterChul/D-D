...
while s != 'q':
    try:
        if s in list_com_1_1:
            list_com_1[s](old_s)
        else:
            s = s.split()
            if s[0] in  list_com_2_1:
                n, ky = list_split[s[0]](s[1:])
                list_com_1[s[0]](int(n), ky)
            elif len(s) == 1:
                if not(test_kyb(int(s[0]),1)):
                    print(error_mess, "Код ошибки: Числа не проходят огранечения 1. Слишком большые или маленькие")
                else:
                    old_s = (1, int(s[0]))
                    kyb(int(s[0]))
            elif len(s) == 2:
                if not(test_kyb(int(s[1]), int(s[0]))):
                    print(error_mess, "Код ошибки: Числа не проходят огранечения 2100. Слишком большые или маленькие")
                else:
                    old_s = (int(s[0]), int(s[1]))
                    kyb(int(s[1]), int(s[0]))
            else:
                print(error_mess,"Ошибка не известна, код 2")
    except:
